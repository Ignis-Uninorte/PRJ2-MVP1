...
interface ProductItemProps {
  ...
  shippingName: string;
  quantity: number;
  id: number;  // Añade el id del producto
}

const ProductItem: React.FC<ProductItemProps> = ({ imgSrc, title, shippingName, quantity, id }) => {
  const [selectedQuantity, setSelectedQuantity] = useState<number>(quantity);
  ...
  return (
    <div className="product-item-sc">
      ...
      <div className="product-details-sc">
        <div className="shipping-method">
          <h3>
            {/* El título ahora es un enlace */}
            <Link to={`/product/Tablets/${id}`}>{title}</Link>
          </h3>
          <h4>Shipping method</h4>
          <label>
            <input type="radio" name={shippingName} value="standard" defaultChecked /> Standard
